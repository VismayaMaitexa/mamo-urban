...
from .models import Cart, Services
...
def add_to_cart_view(request, pk):
    ...
    existing_cart_item = Cart.objects.filter(user=request.user, service=service).first()

    if not existing_cart_item:
        Cart.objects.create(user=request.user, service=service)
        print(f"Service added to cart: {service.skills}")
    
    services = Services.objects.all()
    service_count_in_cart = Cart.objects.filter(user=request.user).count()

    return render(request, 'consumer/consumers_dashboard.html', {
        'services': services,
        'service_count_in_cart': service_count_in_cart
    })
...
def cart_view(request):
    ...
    total = sum(float(item.service.service_rate) for item in cart_items)
    service_count_in_cart = cart_items.count()
...
from .models import Booking
from .forms import PaymentForm
def amountpay_view(request,booking_id):  # Accept id as a parameter
...
def paysuccess_view(request, id):  # Accept the booking id to retrieve payment details
    booking = get_object_or_404(Booking, id=id)  # Retrieve the specific booking
    payment = models.Payment.objects.filter(booking=booking).first()  # Get payment related to this booking

    if not payment:
        return render(request, 'consumer/success.html', {'message': 'No payment details found for this order'})
...
def view_payment_details(request, user_id):  
    payments =models.Payment.objects.filter(booking__user_id=user_id)  
    
    if not payments:
        return render(request, 'consumer/payment_details.html', {'message': 'No payment details found for this user'})
...
def singlepayment_view(request):
    ...
    payments =models.Payment.objects.filter(booking__in=user_bookings)
...
def worker_payment_details(request, worker_id):
    ...
    print(f"Worker ID: {worker_id}")  # Debugging: Print worker_id
    print(f"Worker: {worker}")  # Debugging: Print worker object

    # Retrieve payments related to the specific worker
    payments = Payment.objects.filter(worker=worker)
    print(f"Payments: {payments}")  # Debugging: Print payments
